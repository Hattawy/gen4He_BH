...
#include <iostream>
#include <fstream>
#include "Find_CFF_IM.C"
#include "BH_xsection.C"
#include "asy_alu_cff.C"
...
// Coh : Moh
const double Q2LIMS[2]={1.0,10.0};
const double XBLIMS[2]={0.1,0.7};
const double TLIMS[2]={0.055,0.95};

//InCoh : Moh
//const double Q2LIMS[2]={1.0,5.0};
//const double XBLIMS[2]={0.1,0.55};  // NOTE: xb is for proton
//const double TLIMS[2]={0.06,2.5};

const double VZLIMS[2]={-10.0, 10.0};
...
double xsec_e1dvcs(const dvcsvar_t &x,const dvcspar_t &p)
  {
    return pow(p.q2_0/x.q2,p.alpha) * 1./pow(1.0+p.b*x.t,p.beta) * 1./(1.+pow((x.x-0.3)/p.c,2)) * (1.-p.d*(1.-cos(x.phi)));
 }
...
const dvcspar_t PAR_COHDVCS   = {  1.0,  2.5,   -6.0,  6.5,   0.2,  0.4}; 

//const dvcspar_t PAR_COHDVCS   = {  1.0,  2.5,   -5.0,  7.0,   0.1,  0.4}; 
//const dvcspar_t PAR_COHDVCS   = {  1.0,  2.5,   -11.0/1.35,  12.0/1.35,   0.1,  0.4};  
//const dvcspar_t PAR_COHDVCS   = {  1.0,  2.5,   -11.0,  12.0,   0.1,  0.4}; // b,beta from t-fit to tr

//const dvcspar_t PAR_COHDVCS   = {  1.0,  2.0,  -8.8,     7.3,   0.1,  0.4}; 
//const dvcspar_t PAR_COHDVCS   = {  1.0,  2.0,  -0.667,  35.1,   0.09,  0.4}; 
const dvcspar_t PAR_INCDVCS   = {  1.0,  1.5,  -1.408,   4.0,   0.2,  0.4};
const dvcspar_t PAR_COHPI0    = {  1.0,  3.0,  -8.8,     7.3,   0.3,  0.0};
const dvcspar_t PAR_INCPI0    = {  1.0,  1.5,  -1.3,   1.6,   0.2,  0.0};
...
int main(int argc,char** argv)
{
    const char* usage="\ngen4He [options]\n"
                      ...
                      "\t   2 == Incoherent DVCS\n"
                      "\t   3 == Incoherent Pi0\n"
                      "\t-I Imaginary part of the 4He CFF\n"
                      "\t-R Real part of the 4He CFF\n"
                      "\t-T TYPE\n"
                      "\t   pi0/dvcs+coh/inc\n";
    ...
    int photon_id = 22;
    int proton_id = 2212;
    int alpha_id = 1000020040 ;

    // interpret command line:
    while ( (itmp=getopt(argc,argv,"a:n:s:t:T:o:k:r:I:R:fh")) != -1)
    {
        switch (itmp)
        {
            case 't':
                {
                    ...
                    if (errnno!=0 || ctmp1==ctmp2 || *ctmp2!=0 ||
                            channel<0 || channel>3)
                    {
    ...
    // output lund data files, the first is named as 0
    int n_dat_files = (int) nevents/10000;
    ...
    cout<<"Output Lund Files:      "<<ofile_dat[0]<< " ->  "<<ofile_dat[n_dat_files-1]<<endl<<endl;
    ...
    const double xsecmax = BH_xsection(xmax,xsecpar);
    ...
    while (iev<nevents)
    {
        ...
        double xA1= 1 - xA;
        double y = Q2_1/2/MALPHA/xA/EBEAM;
        double e = 2*xA*MALPHA/sqrt(Q2_1); // epsilon
        double e2 = e*e;
        double Tmin = -1*Q2_1 * (2*xA1*(1-sqrt(1+e2))+e2) / (4*xA*xA1 + e2);
 
        if ( abs(ttt)< abs(Tmin)) continue;
        if ( (1-y + e2*y*y/4) < 0.0 ) continue;

        // sample cross-section:
        dvcsvar_t xsecvar={Q2_1,ttt,xb,phi};
        //const double xsec=xsec_e1dvcs(xsecvar,xsecpar);
        const double xsec=BH_xsection(xsecvar,xsecpar);
        if (xsec>max) max=xsec;
        if (RANDOM.Uniform(0,xsecmax)>xsec) continue;
    
        ffile<<xsec<<"     "<<xsecmax<<endl; 
        const int helicity=RANDOM.Integer(2);

        if (fabs(asymmetry) > 1e-6) {
            
           double samp = 1e12;
           //Find_CFF_IM( xb, ttt, Im, Re); 
           xsecvar.phi = RANDOM.Uniform(0.0,2*TMath::Pi());

            while ( samp > 1.0+(2*helicity-1)*asy_alu(xsecvar,asymmetry,0.))
            //while ( samp > 1.0+(2*helicity-1)*asy_alu_cff(xsecvar, xsecvar.phi, 26.291, -6.3021))
            {
                xsecvar.phi = RANDOM.Uniform(0.0,2*TMath::Pi());
                samp = RANDOM.Uniform(0.0,1.0+asymmetry);
                //samp = RANDOM.Uniform(0.0,1.0+asy_alu_cff(xsecvar, 0.5*TMath::Pi(), 26.291, -6.3021));
             }
        }
        ...
        const int    parent_Z = isInc? 1 : 2;
        const int    target_charge = isInc ? 1 :2; 
        const double nu = Q2_1 / xb / 2 / MPROTON;   
        ...
        //const TLorentzVector p0 = isInc ? p0_proton  : p0_alpha;
        ...
        // if (inRTPC) { p1 = SmearRTPC(p1L);  }
         ...
         if(p1.P() < 0.250) continue;
        
       // if (inRTPC) { if (!AcceptRTPC(p1,v_z))   continue; }
       ...
       // don't do this until necessary (30% faster now):
        ...
        else
        {
            ...
            // and then reconstruct stuff:
             ...
             DVCS_.py_pi0 = pi0.Py();
             DVCS_.pz_pi0 = pi0.Pz();
             DVCS_.the_ggpi0 = g1.Vect().Angle(g2.Vect())*RTODeg;
             ...
             DVCS_.cop_pi0 = GEO.DihedralAngle(q0,p1,-pi0)*RTODeg;
             DVCS_.the_pi0_kpX = pi0.Vect().Angle(pi0X.Vect())*RTODeg;

             DVCS_.dphi_kpi0X_p = GEO.GetPhi(p1X.Phi()-p1.Phi(),-180/RTODeg)*RTODeg;
             DVCS_.the_kpi0X_p = p1X.Vect().Angle(p1.Vect())*RTODeg;
        ...
        const double the_q0g=g.Vect().Angle(q0.Vect())*RTODeg;
        const double thegx=g.Vect().Angle(gammaX.Vect())*RTODeg;
        const double cop=GEO.DihedralAngle(q0,p1,-g)*RTODeg;
        //const double alpha=Nu-sqrt(Nu*Nu+Q2)*cos(the_q0g/RTODeg);
        //const double dphi = GEO.GetPhi(p1X.Phi()-p1.Phi(),-180/RTODeg)*RTODeg;
        //const double tcS= (Q2+2*Nu*alpha)/(1+alpha/target.M());

        const double phi_h = 360-GEO.GetPhi(GEO.DihedralAngle(k1,q0,g),0)*RTODeg; 
        const double phi_h_2 =   GEO.Moh_PHI_H(k0,k1,p1);;
        //const double phi2 =360-GEO.GetPhi(GEO.DihedralAngle(k1,q0,g),0)*RTODeg;
        const double phi3 =180-GEO.GetPhi(GEO.DihedralAngle(k1,q0,p1),-180/RTODeg)*RTODeg;

        DVCS_.hel=helicity;

        DVCS_.phi_h= xsecvar.phi *RTODeg; //phi_h;
        //ffile<< DVCS_.phi_h <<"     "<<phi_h<<"     "<<phi_h_2<<"     "<<phi3<<endl;
        DVCS_.phi2=phi3;
        DVCS_.P0target= p0.P();
        ...
        DVCS_.thegx=thegx;
        DVCS_.therx=p1.Vect().Angle(p1X.Vect())*RTODeg;
        ...
        DVCS_.py_k1=k1.Py();
        DVCS_.pz_k1=k1.Pz();
        DVCS_.phi_k1 = k1.Phi()*RTODeg;
        DVCS_.the_k1 = k1.Theta()*RTODeg;
        ...
        DVCS_.pz_p1=p1.Pz();
        DVCS_.p_p1=p1.P();
        DVCS_.phi_p1 = p1.Phi()*RTODeg;
        DVCS_.the_p1 = p1.Theta()*RTODeg;

        DVCS_.p_p1L=p1L.P();
        DVCS_.phi_p1L = p1L.Phi()*RTODeg;
        DVCS_.the_p1L = p1L.Theta()*RTODeg;

           DVCS_.p1_delta_p = (p1.P() - p1L.P())*1000.0;   // in MeV
           DVCS_.p1_delta_theta = (p1.Theta() - p1L.Theta())*RTODeg;
           DVCS_.p1_delta_phi = (p1.Phi() - p1L.Phi())*RTODeg; 
    ...
    if (DVCS_.ic==1)
     ...
     trk.parent_Z   = parent_Z;
     trk.target_pol = 0;
     trk.beam_pol   = DVCS_.hel;
     trk.x          = DVCS_.xb; 
     trk.y          = DVCS_.y_q0k0;
     trk.W          = DVCS_.wp;    
     trk.Q2         = DVCS_.Q2;
     ...
     // int n_dat_out =  (int) iev/10000;

     // outfile_[n_dat_out]
     //                   <<trk.N_particles<<"   "
     //                   <<trk.parent_A   <<"   "
     //                   <<trk.parent_Z   <<"   "    /* parent charge */
     //                   <<trk.target_pol <<"   "    /* target pol*/
     //                   <<trk.beam_pol   <<"   "    /*beam pol */
     //                   <<trk.x          <<"   "    /* x */
     //                   <<trk.y          <<"   "    /* y  */
     //                   <<trk.W          <<"   "    /* W   */
     //                   <<trk.Q2         <<"   "    /* Q2   */
     ...
     //    outfile_[n_dat_out]                        <<"   "    // Column    Quantity
     //                        <<i+1                  <<"   "    //   1      index
     //                        <<trk.charge[i]        <<"   "    //   2      charge
     //                        <<1                    <<"   "    //   3      type(=1 is active)
     //                        <<trk.Particle_id[i]   <<"   "    //   4      particle id
     //                        <<0                    <<"   "    //   5      parent id (decay bookkeeping)
     //                        <<0                    <<"   "    //   6      daughter (decay bookkeeping)
     //                        <<trk.Px[i]            <<"   "    //   7      px [GeV]
     //                        <<trk.Py[i]            <<"   "    //   8      py [GeV]
     //                        <<trk.Pz[i]            <<"   "    //   9      pz [GeV]
     //                        <<trk.E[i]             <<"   "    //   10     E [GeV]
     //                        <<trk.M[i]             <<"   "    //   11     mass (not used)
     //                        <<trk.X_vertex[i]/100. <<"   "    //   12     x vertex [m]
     //                        <<trk.Y_vertex[i]/100. <<"   "    //   13     y vertex [m]
     //                        <<trk.Z_vertex[i]/100. <<"\n ";    //   14     z vertex [m]
     //   }
